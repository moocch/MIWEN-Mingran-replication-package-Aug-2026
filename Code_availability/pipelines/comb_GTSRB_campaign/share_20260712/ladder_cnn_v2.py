...
def forward(X, P, meta, bn, *, train=False, noise=None):
    """noise: optional per-layer-index rel-RMSE list; adds additive magnitude
    noise (hardware-aware training, run-4 recipe) before BN/bias -- the same
    injection point where the analog error enters the runner."""
    import torch
...
def load_cache(img):
    p = SCRIPT_DIR / f"gtsrb_roi_{img}x{img}.npz"
...
def train_model(arch, *, img=48, epochs=150, seed=0, device="cuda",
    ...
    bn.train(False)
    te = _acc(Xe, ye, P, meta, bn, device)
    te300 = _acc(Xe[:300], ye[:300], P, meta, bn, device)
