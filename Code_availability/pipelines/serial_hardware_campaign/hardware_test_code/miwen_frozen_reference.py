...
def fit_gj_complex(ym: np.ndarray, yr: np.ndarray) -> np.ndarray:
    """Per-column COMPLEX least-squares gain of sync-derotated measured
    tones vs the complex digital reference: g_j = sum(conj(yr)*ym)/
    sum(|yr|^2). Absorbs per-bin amplitude ripple AND phase response.
    Calibration-time only (reference computed on the disjoint set)."""
    num = np.sum(np.conj(yr) * ym, axis=0)
    den = np.maximum(np.sum(np.abs(yr) ** 2, axis=0), 1e-30)
    g = num / den
    return np.where(np.abs(g) > 1e-12, g, 1.0)
...
def run_layer_hw(core, W, X, *, args, layer_tag, hw, g_frozen,
    ...
    for c0 in range(0, S, per):
        ...
        # ADC-clip gate (2026-08-17 audit finding 2): clipping must be loud.
        if res.clip_frac > 1e-2:
            raise RuntimeError(f"ADC clip L{layer_tag}@{c0}: "
                               f"clip_frac={res.clip_frac:.4f} > 1e-2")
        if res.clip_frac > 0:
            print(f"[clip] L{layer_tag}@{c0}: {res.clip_frac*100:.4f}%",
                  flush=True)
        ym = res.y_meas
        if readout == "re":
            # remove the random per-capture global phase via the sync
            ym = ym * np.exp(-1j * res.sync_phase)
        Y[idx] = ym
        s2s.append(res.sigma2_bin)
    sigma2 = float(np.mean(s2s))
    if readout == "re":
        # complex frozen calibration; zero-mean noise -> NO debias
        return np.real(Y / g_frozen[None, :]), sigma2
