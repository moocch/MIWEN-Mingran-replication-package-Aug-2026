...
# N=65536 配套参数 (相对原 N=4096 整体 x16, 保持完全相同的频域几何):
#   硬约束: 梳最大占用 |k|max = 3N/2 + k0 - 2 < L/2  =>  L 最小可取 2^18 = 262144
#           (占用 75% Nyquist, 与 N=4096/L=16384 时完全一致)
#   CP = L/32, gap = L/2 维持原比例; k0 同步 x16 (6->96, 仍 ≡0 mod 3),
#   使 IP 落点绝对频率不变 (fs=10M 时仍为 k0*fs/L ≈ +3.66 kHz, 远离 DC 附近的
#   1/f 噪声与直流校正残留 —— 若 k0 保持 6, IP 会落到离 DC 仅 ~229 Hz 处).
N_DEFAULT = 65536
FFT_LEN_DEFAULT = 262144
CP_LEN_DEFAULT = 8192
COMB_M = 3
K0_DEFAULT = 96
Q_DATA_DEFAULT = 200
PILOT_EVERY_DEFAULT = 8
GAP_SAMPLES_DEFAULT = 131072
SYNC_BAND_FRAC = 8
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--papr-clip-db", type=float, default=10.0,
                   help="TX 数字削波 PAPR (dB), 0=不削波. N=65536 不削波时 PAPR≈16 dB, "
    ...
    p.add_argument("--save-vectors", action="store_true",
                   help="把全部 data 向量存进 NPZ (N=65536 时 ~200 MB; 默认只存种子, 可完整重建)")
