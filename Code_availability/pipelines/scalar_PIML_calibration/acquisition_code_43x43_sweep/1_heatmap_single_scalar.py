...
----
    ...
    #    默认输出到脚本同目录 data/:
    #    usrp_mixer_heatmap_YYYYMMDD_HHMMSS.npz / .png
    ...
    # 4. 已有 NPZ 重画
    python usrp_mixer_heatmap.py --replot data/today_YYYYMMDD_HHMMSS.npz --out-png today_v2.png
...
DEFAULT_OUTPUT_STEM = "usrp_mixer_heatmap"
...
def resolve_output_path(
    ...
    *,
    ...
    """
    Put relative outputs under this script's data/ directory and append timestamp.

    Absolute paths are respected, but still receive the same timestamp suffix.
    """
    raw_path = Path(out_path)
    path = raw_path if raw_path.is_absolute() else DATA_DIR / raw_path
    ...
    for idx in range(1, 1000):
        numbered = candidate.with_name(f"{stem}_{timestamp}_{idx:03d}{suffix}")
        if not numbered.exists():
            return numbered

    raise RuntimeError(f"failed to create unique output path for {candidate}")


def resolve_input_path(in_path: str | Path) -> Path:
    """Resolve replot inputs, checking data/ for relative paths as a fallback."""
    path = Path(in_path)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--out-npz",
        type=str,
        default=f"{DEFAULT_OUTPUT_STEM}.npz",
        help="输出 NPZ 文件名/路径. 相对路径会写到脚本同目录 data/ 下, 并自动加时间戳.",
    )
    p.add_argument(
        "--out-png",
        type=str,
        default=f"{DEFAULT_OUTPUT_STEM}.png",
        help="输出 PNG 文件名/路径. 相对路径会写到脚本同目录 data/ 下, 并自动加时间戳.",
    )
